    ...
    if (isLoading) {
        return (
            <div className="space-y-8 pb-10 animate-pulse">
                <div className="h-32 bg-[var(--surface)] rounded-xl"></div>
        ...
        <div className="space-y-8 pb-10">
                ...
                {safeErrors.length > 0 ? (
                    <div className="space-y-2 mt-4">
                        {safeErrors.map((error) => (
                            <div key={error.id} className="group bg-[var(--bg)]/50 border border-[var(--border)] rounded-xl p-5 hover:border-status-error/30 transition-all">
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-start gap-4">
                                        ...
                                        <div className="space-y-1">
                                            <div className="flex items-center gap-3">
                                                <h4 className="text-base font-bold text-[var(--text)]">{error.pipeline}</h4>
                                            ...
                                            <div className="flex items-center gap-3 text-[11px] text-[var(--text-muted)] mt-2 uppercase tracking-wider font-medium">
                                    ...
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="h-8 w-[1px] bg-[var(--border)] mx-2 hidden md:block" />
                    ...
                    <div className="h-64 flex flex-col items-center justify-center border-2 border-dashed border-[var(--border)] mt-4 rounded-xl">
                        <AlertTriangle className="text-[var(--text-muted)] mb-2 w-12 h-12 opacity-50" />
                        <p className="text-[var(--text-muted)] font-medium">No errors detected. Systems are green.</p>
                    </div>
                )}
            </SectionCard>
            ...
            <div className="p-6 bg-[var(--surface)] border border-[var(--border)] rounded-xl">
                <div className="flex items-center justify-between">
                    <div className="space-y-1">
                        <h5 className="font-bold text-[var(--text)]">Need technical assistance?</h5>
                        <p className="text-sm text-[var(--text-muted)]">Our DevOps team is available for pipeline debugging and LLM fine-tuning.</p>
                    </div>
